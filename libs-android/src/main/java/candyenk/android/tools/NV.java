 ...
 * Nimble
 * 集天下之大成
 * 所有控件都能用，建议灵动控件的父级留出灵动空间，设置Padding
 */
public class NV implements View.OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent e) {
        switch (e.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                switch (tState) {
                    case 0, 1 -> this.tState = 2;
                    case 2 -> Nimble(v, e.getX(), e.getY());
                }
                return false;
    ...
    private void NimbleDown(View v, MotionEvent e) {
        ...
        float fx = b ? 0 : Math.clamp((h - y) / h * 7f, -7f, 7f);
        float fy = b ? 0 : Math.clamp((w - x) / w * -7f, -7f, 7f);
    ...
    private void Nimble(View v, float x, float y) {
        ...
        int w = v.getWidth() / 2;
        int h = v.getHeight() / 2;
        float fx = Math.clamp((h - y) / h * 7f, -7f, 7f);
        float fy = Math.clamp((w - x) / w * -7f, -7f, 7f);
        v.setRotationX(fx);
        v.setRotationY(fy);
    }
