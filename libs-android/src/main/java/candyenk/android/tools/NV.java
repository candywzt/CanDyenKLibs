...
public class NV implements View.OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: // 手指按下(0)
                this.touchState = 1;
                NimbleAnimationDown(v, event);
                if (v.hasOnClickListeners()) return false;
                else return Build.VERSION.SDK_INT < Build.VERSION_CODES.R || !v.hasOnLongClickListeners();
    ...
    private void NimbleAnimationDown(View view, MotionEvent event) {
        ...
        if (x > w2 * 4 / 5f && x < w2 * 8 / 5f && y > h2 * 4 / 5f && y < h2 * 8 / 5f)
